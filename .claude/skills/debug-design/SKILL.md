...
1. **Macro placement failures (MPL-0040 or similar)**: The macro placer cannot find valid placements.
   ...
   - **If the design has many FakeRAM macros and platform-knob tuning isn't unsticking it**, consider repartitioning — splitting wide banks into more numerous narrow ones (or merging them) often fixes macro-driven floorplan failures that no halo/density value can.  See `.claude/skills/shared/sram-repartition.md`.
