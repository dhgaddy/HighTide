...
- **Still non-finishing** on this pin: litepci-sky130hd (GRT congestion), NVDLA partition_c-sky130hd (GP plateau), partition_c-sky130hd / partition_o-sky130hd (detail-route congestion). The new resizer's `place_gp` runs two timing-driven `repair_design` passes, making 1.3M+ cell designs ~2–3× slower (snitch-nangate45 ~14 h end-to-end).
