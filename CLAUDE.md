...
| **MPL-0040** — `rtl_macro_placer` annealing failure on certain macro clusters | asap7 `bp_quad` (pipe_fma cluster) | Hand-place fakeram macros in left-edge columns at FIRM status via `macros.tcl` so RTLMP only sees pre-placed macros | `09542e19` | [OpenROAD#9985](https://github.com/The-OpenROAD-Project/OpenROAD/issues/9985) |
...
| **DPL-0036** in CTS-internal `detailed_placement` — `cts.tcl` builds its own `dpl_args` ignoring `DETAIL_PLACEMENT_ARGS`, so CTS-inserted leaf clock buffers / repair_clock_nets buffers can land inside large SRAM macro bodies far from any legal stdcell row | asap7 `snitch_cluster`; gt2n `cnn` | `PRE_CTS_TCL` wraps `detailed_placement` to inject `-max_displacement`. snitch_cluster: `{2000 400}`. gt2n/cnn: `{400 100}` (observed max displacement 349.5µm from buffers landing inside 175µm-deep w16_l32768 SRAM bodies) | `0af20020` | None (ORFS layering issue; no upstream issue filed yet) |
...
| **DRT-0406** "No {horizontal\|vertical} tracks found" in track assignment (`FlexTAWorker::assignIroute_availTracks`) — `dbBlock::getGCellTileSize()` sizes the global-routing GCell tile only from the median pitch of M2/M3/M4 × 15, independent of which layers are actually enabled. On a technology whose top routing layer's pitch is far coarser than its bottom layers (gt2n: M12 pitch 0.72 µm vs M2 0.024 µm, ~30x), the GCell tile (0.42 µm here) is smaller than the top layer's own pitch, so ~40% of GCells contain zero tracks on that layer; if global routing ever assigns a wire to one of those tiles, track assignment hard-fails. Only manifests when GRT's solution actually uses the coarse top layer for some net — gt2n `cnn` never exercises M12/M13 (0 demand in its congestion report) so it never hit this; gt2n `NVDLA/partition_o` did (M12 usage 2.23%) and failed after ~3.5h in route | gt2n `NVDLA/partition_o` (`MAX_ROUTING_LAYER=M13`) | `patches/openroad-gcell-tile-size-fix.patch` — after computing the existing median(M2,M3,M4)×15 tile size, also floor it at 2× the actual top enabled routing layer's own pitch, so every GCell has track coverage on every enabled layer. No-op for designs/platforms where the bottom-layer-derived tile is already ≥ that (i.e. everything with a normal bottom-to-top pitch ratio) | (this PR) | None filed upstream |
