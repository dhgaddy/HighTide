...
- **2026-07-07 (die sizing — asap7→gt2n area ratio ~4.1×)**: floonoc on asap7 has a core area of ~576 µm². gt2n standard cells are physically larger relative to the routing pitch, and the fine M2/M3 pitch raises routing demand per unit area — the expected scaling factor is ~4.1× asap7. Calibrated empirically through build iterations:
  - v1 (80×80 die, 72×72 core = 5,184 µm²): 84% utilization — DRT oscillated at ~1,900 violations.
  - v2 (92×92 die, 86×86 core): DRT oscillated at ~530 violations even with the correct M2/M3 io.tcl.
  - v3–v5 (progressive upsizing): congestion persisted; DRT violation counts dropped with each size increase but never reached zero.
  - **v6 (129×129 die, core 6–123 = 117×117 µm, 13,689 µm²)**: DRT converged cleanly. Core area ≈ 4.1× asap7; die held stable through all subsequent clock-tightening builds. The 6 µm margin on each side provides a full usable border row.
...
- **2026-07-07 (clock tightening — HighTide loose-first loop)**: after the die size stabilized at v6, the HighTide tightening loop was applied (target = period_min × 1.10, iterate until ratio ∈ [1.05, 1.15]). Started from a loose 700 ps constraint and tightened over builds v7–v11. Converged at **1151 ps** (period_min 1089.54 ps, ratio 1.056, WNS = 0, 0 DRC violations, Fmax 917 MHz).
